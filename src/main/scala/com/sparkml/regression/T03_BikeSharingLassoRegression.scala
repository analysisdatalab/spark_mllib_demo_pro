...
/**
 * 回归算法--线性回归--共享单车案例--使用LassoWithSGD算法回归模型预测每小时单车租用数量
 *
 * 一般来说，线性回归都伴随这随机梯度下降，目的是对模型进行优化
 *
 */
object T03_BikeSharingLassoRegression {
  ...
  def main(args: Array[String]): Unit = {
     ...
     * ): LassoModel
     */
    val lassoModel = LassoWithSGD.train(trainRDD, 100, 1.0, 0.01, 1.0)

    //进行预测,返回元组（预测值，实际值）
    val lassoPredictAndActualRDD = testRDD.map { case LabeledPoint(label, features) => (lassoModel.predict(features), label) }
    //需要有一个回归模型评估器，对模型预测的结果进行评估,回归模型的参考指标RMSE(均方根误差)，MAE (Mean Absolute Error 平均绝对值误差)
    val lassoMetrics = new RegressionMetrics(lassoPredictAndActualRDD)
    println("Lasso回归 RMSE=" + lassoMetrics.rootMeanSquaredError)
    println("Lasso回归 MAE=" + lassoMetrics.meanAbsoluteError)
