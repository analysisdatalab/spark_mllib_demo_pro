...
public class T03_SaveModel {
    public static void main(String[] args) throws IOException {
        Logger.getLogger("org").setLevel(Level.WARN);
        // 1.初始化spark配置信息并建立与spark的连接
        SparkSession spark = SparkSession.builder().appName("T03_SaveModel").master("local").getOrCreate();

        //2， 创建数据集并分词
        RDD<String> rdd = spark.sparkContext().textFile("data/ml/crossvalidation/news_train", 2);
        JavaRDD<Row> parsedRDD = rdd.toJavaRDD().map(line -> {
            ...
            return RowFactory.create(label, splits[1].split(" "));
        });
        ...
        int[] layers = new int[] {VECTOR_SIZE, 6, 5, 10};
        ...
        // 对管道Pipeline进行持久化
        pipeline.write().overwrite().save("ml_model/pipeLine");

        PipelineModel model = pipeline.fit(trainingData);
        // 也可以对模型进行持久化
        model.write().overwrite().save("ml_model/pipeLineModel");
